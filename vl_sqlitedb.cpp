...
int SQLiteDB::addTransaction(const QString &t_transactionName, const QString &t_recordName, const QString &t_contentSet)
{
    int retVal = -1;
    int recordId = 0;
    ...
    m_dPtr->m_transactionInsertQuery.bindValue(":contentset_name", t_contentSet);
...
bool SQLiteDB::openDatabase(const QString &t_dbPath)
{
    ...
    if(fInfo.absoluteDir().exists())
    {
        ...
        else
        {
           ...
           */
                m_dPtr->m_transactionInsertQuery.prepare("INSERT INTO transactions (id, recordsid, transaction_name, contentset_name, start_time, stop_time) VALUES (:id, :recordsid, :transaction_name, :contentset_name, :start_time, :stop_time);");
