    ...
    m_dbLogger(dbLogger)
{
    connect(m_dbLogger, &VeinLogger::DatabaseLogger::sigTakeSnapshotCompleted,
            this, &RpcCreateSnapshot::onCreateSnapshotCompleted, Qt::QueuedConnection);
}
...
void RpcCreateSnapshot::callRPCFunction(const QUuid &callId, const QVariantMap &parameters)
{
    ...
    if(errs.isEmpty())
        m_dbLogger->startTakeSnapshot(callId, transactionName, sessionName, contentSets, guiContext);
    else
